...
void drawQuadWithScene2() {
    ...
    bloomFinal->use();
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, textureCatalog->get(Texture::REFLECTION_RENDER_TEXTURE)->getId());
    bloomFinal->addUniform("scene", 0); // 0 because GL_TEXTURE0
    glActiveTexture(GL_TEXTURE1);
    glBindTexture(GL_TEXTURE_2D, textureCatalog->get(Texture::NOODLE_TEXTURE)->getId());
    bloomFinal->addUniform("bloomBlur", 1); // 1 because GL_TEXTURE1
    renderQuad();
    bloomFinal->stop();
}
