...
void keyRelease( unsigned char key, int x, int y ){
    ...
    if( KeyBuffer::instance()->isKeyDown( 'm' ) || KeyBuffer::instance()->isKeyDown( 'M' ) ){
        ...
        switch( mode ){
            case 0: std::cout << "Mode Normal" << std::endl;
                break;
            case 1: std::cout << "Mode Position in ViewSpace" << std::endl;
                break;
            case 2: std::cout << "Mode Normals in ViewSpace" << std::endl;
                break;
            case 3: std::cout << "Mode Basic Object Color" << std::endl;
                break;
            case 4: std::cout << "Mode Brightness Unblurred" << std::endl;
                break;
            case 5: std::cout << "Mode Brightness Blurred" << std::endl;
                break;
            case 6: std::cout << "Mode Occlusion" << std::endl;
                break;
            case 7: std::cout << "Mode Occlusion Blurred" << std::endl;
                break;
            case 8: std::cout << "Mode Position in WorldSpace" << std::endl;
                break;
            case 9: std::cout << "Mode Normals in WorldSpace" << std::endl;
                break;
            case 10: std::cout << "Mode Normal but unblurred occlusion" << std::endl;
                break;
            case 11: std::cout << "Mode Normal but no occlusion" << std::endl;
                break;
        }
    }
...
unsigned int gPosition, gNormal, gAlbedoSpec, gBright, gPositionViewSpace, gNormalViewSpace;
...
void createGBuffer(){
    ...
    // - position in ViewSpace color buffer
    glGenTextures( 1, &gPositionViewSpace );
    glBindTexture( GL_TEXTURE_2D, gPositionViewSpace );
    glTexImage2D( GL_TEXTURE_2D, 0, GL_RGB16F, WinX, WinY, 0, GL_RGBA, GL_FLOAT, NULL );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST );
    glFramebufferTexture2D( GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT4, GL_TEXTURE_2D, gPositionViewSpace, 0 );

    // - normals in ViewSpace color buffer
    glGenTextures( 1, &gNormalViewSpace );
    glBindTexture( GL_TEXTURE_2D, gNormalViewSpace );
    glTexImage2D( GL_TEXTURE_2D, 0, GL_RGB16F, WinX, WinY, 0, GL_RGBA, GL_FLOAT, NULL );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST );
    glFramebufferTexture2D( GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT5, GL_TEXTURE_2D, gNormalViewSpace, 0 );
...
void drawDeferredScene(){
    ...
    glActiveTexture( GL_TEXTURE16 );
    glBindTexture( GL_TEXTURE_2D, gPositionViewSpace );
    renderShader->addUniform( "gPositionViewSpace", 16 );

    glActiveTexture( GL_TEXTURE17 );
    glBindTexture( GL_TEXTURE_2D, gNormalViewSpace );
    renderShader->addUniform( "gNormalViewSpace", 17 );
