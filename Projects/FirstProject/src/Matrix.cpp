...
namespace engine {
    ...
    const mat4 MatrixFactory::createReflectionMatrix(vec4 rp) {
        mat4 res = mat4(1 - 2 * rp.x * rp.x, -2 * rp.x*rp.y, -2 * rp.x * rp.z, -2 * rp.x * rp.w,
                        -2 * rp.y * rp.z, 1 - 2 * rp.y * rp.y, -2 * rp.y * rp.z, -2 * rp.y * rp.w,
                        -2 * rp.z * rp.x, -2 * rp.z * rp.y, -2 * rp.z * rp.z + 1, -2 * rp.z * rp.w,
                        0, 0, 0, 1);

        //mat4 res = mat4(- 2 * rp.x * rp.x + 1, -2 * rp.y*rp.x, -2 * rp.z * rp.x, 0,
        //                 -2 * rp.x * rp.y, - 2 * rp.y * rp.y + 1, -2 * rp.z * rp.y, 0,
        //                 -2 * rp.x * rp.z, -2 * rp.y * rp.z, - 2 * rp.z * rp.z + 1, 0,
        //                -2 * rp.x * rp.w, -2 * rp.y * rp.w, -2 * rp.z * rp.w, 1 );
        res.clean();
        return res;
    }

    const mat4 MatrixFactory::createReflectionMatrix2(vec4 rp, vec3 p) {
        mat4 res = mat4(1 - 2 * rp.x * rp.x, -2 * rp.x*rp.y, -2 * rp.x * rp.z, 2 * p.x * rp.w,
                        -2 * rp.y * rp.z, 1 - 2 * rp.y * rp.y, -2 * rp.y * rp.z, 2 * p.y * rp.w,
                        -2 * rp.z * rp.x, -2 * rp.z * rp.y, -2 * rp.z * rp.z + 1, 2 * p.z * rp.w,
                        0, 0, 0, 1);
